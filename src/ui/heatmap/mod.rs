...
fn render_heatmap_body(
    ...
    let heatmap_inner = panels::render_heatmap_frame(f, area);
    ...
    let render_key = HeatmapRenderKey {
        ...
        segment_start: page_window
            .as_ref()
            .map_or(0, |window| window.current_range().0),
        segment_len: page_window.as_ref().map_or(0, |window| {
            window
                .current_range()
                .1
                .saturating_sub(window.current_range().0)
        }),
    ...
    if let Some(cache_index) = cached_pos {
        let (slice_summary, legend_summary, viewport_selection, selection) = {
            let entry = state
                .heatmap_render
                .cached_pages
                .get_mut(cache_index)
                .expect("heatmap cache entry should exist");
            ...
            (slice_summary, legend_summary, viewport_selection, selection)
        };
        ...
        panels::render_heatmap_loading(f, &heatmap_inner);
        if let Some(sidebar_area) = sidebar_area {
            panels::render_heatmap_loading_sidebar(f, sidebar_area)?;
        }
    }
